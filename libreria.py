...
def get_task(ip_address, force=[]):
	...
	with connection.cursor() as cursor:
		...
		if len(alts)==1: sql = "select * from tasks_abejas where (task_completed=0 and task_responsible=\"\") or (task_id in ("+", ".join(alts)+")) order by RAND() limit 1000;" 
		else: sql = "select * from tasks_abejas where (task_id in ("+", ".join(alts)+")) order by RAND() limit 1000;" 
		print(sql)
		cursor.execute(sql)
...
def get_connection():
	return pymysql.connect(host='ec2-3-128-18-58.us-east-2.compute.amazonaws.com',
							 user='root',
							 password='password',
							 db='db-name',
							 charset='utf8mb4',
...
def get_geodata(coordenadas, margin, year):
